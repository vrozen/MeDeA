...
module lang::mpl::Syntax

start syntax MPL
  = pttr: "pattern" TID "(" {NID ","}* ")" "intent" String "useWhen" String "{" Element* "}";
...
public start[MPL] mpl_parse(str src, loc file) = 
  parse(#start[MPL], src, file);
  
public start[MPL] mpl_parse(loc file) = 
  parse(#start[MPL], file); 